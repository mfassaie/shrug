//! Confluence space property sub-entity: list, create, view, edit, delete.
//!
//! All operations use the v2 API at /wiki/api/v2/spaces/{spaceId}/properties.
...
/// Space property subcommands.
#[derive(Subcommand)]
pub enum SpacePropertyCommands {
    /// List properties on a space
    List {
        /// Space ID
        space_id: String,
    },
    ...
    /// View a specific space property
    View {
        /// Space ID
        space_id: String,
        /// Property ID
    ...
    /// Edit a space property (auto-increments version)
    Edit {
        /// Space ID
        space_id: String,
        /// Property ID
...
/// Fetch a space property to extract its current version number.
...
/// Execute a space property command.
pub fn execute(
    cmd: &SpacePropertyCommands,
    credential: &ResolvedCredential,
    client: &Client,
    base_url: &str,
    ...
    match cmd {
        SpacePropertyCommands::List { space_id } => {
            let url = format!(
                ...
                match output_format {
                    ...
                    _ => {
                        if let Some(id) = json_val.get("id").and_then(|v| v.as_str()) {
                            println!("Created space property {} with key \"{}\"", id, key);
                        } else {
                            println!("Created space property with key \"{}\"", key);
                        }
                    }
                }
            }
        ...
        SpacePropertyCommands::View {
            space_id,
            property_id,
        } => {
            let url = format!(
                "{}/wiki/api/v2/spaces/{}/properties/{}",
                base_url, space_id, property_id
            );
        ...
        SpacePropertyCommands::Edit {
            ...
            match output_format {
                ...
                _ => println!(
                    "Updated space property {} (version {})",
        ...
        SpacePropertyCommands::Delete {
            ...
            match output_format {
                ...
                _ => println!("Deleted space property {}", property_id),
            }
            Ok(())
        }
    }
...
mod tests {
    ...
    fn test_space_property_url() {
        ...
        assert_eq!(
            url,
            "https://site.atlassian.net/wiki/api/v2/spaces/12345/properties"
        );

        let view_url = format!(
            "{}/wiki/api/v2/spaces/{}/properties/{}",
            "https://site.atlassian.net", "12345", "prop-1"
        );
